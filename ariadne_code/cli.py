...
def _parse_args():
    ...
    p.add_argument(
        ...
        default="full",
        help="Agent profile: full coding agent, read-only Q&A, or read-only review mode.",
    )
...
def main():
    ...
    agent = Agent(
        llm=llm,
        tools=tools_for_profile(profile.name),
...
def _repl(agent: Agent, config: Config):
    ...
    while True:
        ...
        if user_input == "/tokens":
            ...
            continue
        if user_input == "/tools":
            console.print("[bold]Available tools in this profile:[/bold]")
...
def _show_help():
    console.print(Panel(
        ...
        "  /tools         Show tools available in this profile\n"
